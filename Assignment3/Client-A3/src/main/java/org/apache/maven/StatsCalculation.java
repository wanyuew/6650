...
public class StatsCalculation {
    ...
    public static void calculateStats(List<ResponseTimeData> allResponseTimes) {
        ...
        List<ResponseTimeData> postAlbumResponseTimes = allResponseTimes.stream()
                .filter(data -> "PostTAlbum".equals(data.getRequestType()))
                .collect(Collectors.toList());

        List<ResponseTimeData> postActionResponseTimes = allResponseTimes.stream()
                .filter(data -> "PostAction".equals(data.getRequestType()))
                .collect(Collectors.toList());

        // Calculate statistics for POST requests
        calculateAndPrintStats(postAlbumResponseTimes, "Post Album");

        // Calculate statistics for GET requests
        calculateAndPrintStats(postActionResponseTimes, "Post Action");
    }
